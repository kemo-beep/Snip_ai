...
export default function WorkingTimeline({
    ...
    onDeleteClip,
    onDuplicateClip
}: WorkingTimelineProps) {
    const [zoom, setZoom] = useState(1)
    const [isSnapping, setIsSnapping] = useState(true)
    const [showGrid, setShowGrid] = useState(true)
    const [selectedClips, setSelectedClips] = useState<string[]>([])

    // Create timeline data - FORCE both tracks to always exist
    const timelineData: TimelineRow[] = useMemo(() => {
        ...
        // ALWAYS create both tracks, even if empty
    ...
    console.log('Rendering WorkingTimeline with data:', { timelineData, effects, clips })
